...
namespace audio {
    ...
    public:
        PeerConnectionObserverImpl(audio::instance_ptr inst) {
            inst_ = inst;
            csd_observ_ = new audio::CreateSessionDescriptionObserverImpl(this);
            ssd_observ_ = new audio::SetSessionDescriptionObserverImpl(this);
            ssd_observ_local_ = new audio::SetSessionDescriptionObserverLocalImpl(this);
        }
        ...
        ) override
        {
            ...
            if (candidate->ToString(&sdp)) {
                nlohmann::json json = {{"type", "candidate"}, {"candidate", sdp}};
                {
                    ...
                    audio::events_websockets_to_webrtc_.push({EVENT_TYPE_CANDIDATE, json.dump()});
                }
            }
        }
    private:
        audio::CreateSessionDescriptionObserverImpl* csd_observ_;
        audio::SetSessionDescriptionObserverImpl* ssd_observ_;
        audio::SetSessionDescriptionObserverLocalImpl* ssd_observ_local_;
        audio::instance_ptr inst_;
    };
};